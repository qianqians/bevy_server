...
use mongodb::bson::{ doc, Bson, Array, Document};
...
impl MongoProxy {
    ...
    async fn create_index(&mut self, db: String, collection: String, key: String, is_unique: bool) -> bool {
    ...
    async fn check_int_guid(&mut self, db: String, collection: String, initial_guid: u32) -> bool {
        let _db = self.client.database(&db);
        let _collection  = _db.collection::<Document>(&collection);
    ...
    async fn save(&mut self, db: String, collection: String, data: Vec<u8>) -> bool {
        let _db = self.client.database(&db);
        let _collection  = _db.collection::<Document>(&collection);
    ...
    async fn update(&mut self, db: String, collection: String, query: Vec<u8>, update: Vec<u8>, is_upsert: bool) -> bool {
        let _db = self.client.database(&db);
        let _collection  = _db.collection::<Document>(&collection);
    ...
    async fn find_and_modify(&mut self, db: String, collection: String, query: Vec<u8>, update: Vec<u8>, _new: bool, _upsert: bool) -> bool {
        let _db = self.client.database(&db);
        let _collection  = _db.collection::<Document>(&collection);
    ...
    async fn find(&mut self, db: String, collection: String, query: Vec<u8>, skip: u32, limit: u32, sort: String, _ascending: bool) -> Result<Vec<Document>, Box<dyn std::error::Error> >  {
        let _db = self.client.database(&db);
        let _collection  = _db.collection::<Document>(&collection);
    ...
    async fn count(&mut self, db: String, collection: String, query: Vec<u8>) -> i32 {
        let _db = self.client.database(&db);
        let _collection  = _db.collection::<Document>(&collection);
    ...
    async fn remove(&mut self, db: String, collection: String, query: Vec<u8>) -> bool {
        let _db = self.client.database(&db);
        let _collection  = _db.collection::<Document>(&collection);
    ...
    async fn get_guid(&mut self, db: String, collection: String) -> i32 {
        let _db = self.client.database(&db);
        let _collection  = _db.collection::<Document>(&collection);
        ...
        let guid_result = _guid_doc.get_i32("guid");
        match guid_result {
            Ok(_v) => return _v,
            Err(_e) => return -1
        }
